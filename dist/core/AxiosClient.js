...
const AxiosCookie_1 = require("./AxiosCookie");
...
class AxiosClient {
    ...
    */
    constructor(config = {
        cookie: true,
        jar: new AxiosCookie_1.AxiosCookie().toJSON(),
        headers: {},
    }) {
        this.classId = '@ing3kth/core/AxiosClient';
        if (config.cookie) {
            this.jar = AxiosCookie_1.AxiosCookie.fromJSON(config.jar);
            this.axiosClient = (0, axios_cookiejar_support_1.wrapper)(axios_1.default.create({ jar: this.jar, withCredentials: true, headers: config.headers }));
        }
        else {
            this.axiosClient = axios_1.default.create({ httpsAgent: new https.Agent({ rejectUnauthorized: false }), headers: config.headers });
        }
    }
    /**
